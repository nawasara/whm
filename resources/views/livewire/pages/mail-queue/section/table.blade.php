<div wire:init="loadQueue">
    ...
    @else
        @php
            $statusOptions = ['queued' => 'Queued', 'deferred' => 'Deferred', 'frozen' => 'Frozen'];
            $ageOptions = ['1h' => '> 1 jam', '24h' => '> 24 jam', '7d' => '> 7 hari'];
        @endphp

        {{-- Page header — title + Refresh + Flush All. Mail queue is
             live data from Exim (not DB), so no export. --}}
        <x-nawasara-ui::page-header
            ...
            @can('whm.mailqueue.manage')
                <x-nawasara-ui::button color="warning" variant="outline" size="sm"
                    wire:click="flushAll"
                    wire:confirm="Flush seluruh queue? Exim akan coba kirim semua message sekarang.">
        ...
        {{-- Server selector + status summary cards. Server is a hard
             scope. Status counts surface as visual stat-cards (compact
             mode) so users see distribution without applying a filter. --}}
        <div class="flex flex-wrap items-center gap-3 mb-4">
            <x-nawasara-whm::server-switcher :servers="$this->servers" role="mail" />
        </div>

        <div class="grid grid-cols-2 md:grid-cols-3 gap-2 mb-4">
            <x-nawasara-ui::stat-card compact label="Queued"   :value="$this->statusCounts['queued']   ?? 0" color="info" />
            <x-nawasara-ui::stat-card compact label="Deferred" :value="$this->statusCounts['deferred'] ?? 0" color="warning" />
            <x-nawasara-ui::stat-card compact label="Frozen"   :value="$this->statusCounts['frozen']   ?? 0" color="danger" />
        </div>

        {{-- Toolbar — Status multi-select + Age single-select + search. --}}
        <div class="space-y-2 mb-4">
            <div class="flex flex-col md:flex-row md:flex-nowrap md:items-center gap-2">
                <div class="flex flex-wrap items-center gap-2 shrink-0">
                    <x-nawasara-ui::filter-panel
                        label="Filter"
                        :state="['statusFilter' => $statusFilter, 'ageFilter' => $ageFilter]"
                        :multiple="['statusFilter']"
                        :labels="['statusFilter' => $statusOptions, 'ageFilter' => $ageOptions]"
                        :dimensions="['statusFilter' => 'Status', 'ageFilter' => 'Age']">
                        <x-nawasara-ui::filter-group label="Status" model="statusFilter" :items="$statusOptions" icon="lucide-circle-check" />
                        <x-nawasara-ui::filter-group label="Age" model="ageFilter" :items="$ageOptions" icon="lucide-clock" />
                    </x-nawasara-ui::filter-panel>
                </div>
